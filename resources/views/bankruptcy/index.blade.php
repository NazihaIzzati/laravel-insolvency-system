    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="professional-section">
            <div class="professional-section-header">
                <h3 class="text-lg font-medium text-neutral-900">Bankruptcy Records</h3>
                <p class="text-sm text-neutral-700 mt-1">All individual bankruptcy records</p>
            </div>
            <div class="professional-section-content">
                <!-- Search Form -->
                <div class="bg-white rounded-lg border border-neutral-200 p-6 mb-6 shadow-sm">
                    ...
                    <form id="bankruptcySearchForm" class="space-y-4">
                        @csrf
                        <div>
                            <label for="bankruptcy_search_input" class="block text-sm font-medium text-neutral-700 mb-2">
                                Search Records
                            </label>
                            <div class="relative">
                                ...
                                <input type="text" 
                                       ...
                                       required>
                                <button type="button" id="clearSearchBtn" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-800 hover:text-neutral-800 transition-colors duration-200 p-1 rounded-full hover:bg-gray-100 cursor-pointer z-10" style="display: none;" title="Clear search">
                ...
                <!-- Loading Spinner -->
                <div id="bankruptcyLoadingSpinner" class="hidden bg-white rounded-lg border border-neutral-200 p-8 mb-6 shadow-sm">
                ...
                <!-- Search Results -->
                <div id="bankruptcySearchResults" class="hidden bg-white rounded-lg border border-neutral-200 mb-6 shadow-sm">
                    <div class="px-6 py-4 border-b border-neutral-200">
                        <div class="flex items-center justify-between">
                            ...
                            <button type="button" id="clearBankruptcySearchResultsBtn" class="text-sm text-neutral-700 hover:text-neutral-700 flex items-center">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-white-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-neutral-700 uppercase tracking-wider">IC No</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-neutral-700 uppercase tracking-wider">Court Case No</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-neutral-700 uppercase tracking-wider">RO Date</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-neutral-700 uppercase tracking-wider">AO Date</th>
                ...
                <!-- No Results -->
                <div id="bankruptcyNoResults" class="hidden bg-white rounded-lg border border-neutral-200 p-8 mb-6 shadow-sm">
                    <div class="text-center">
                    ...
                    @if($bankruptcies->count() > 0)
                        ...
                        <table class="min-w-full divide-y divide-primary-200" style="min-width: 1200px;">
                            <thead class="bg-neutral-50">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-neutral-700 uppercase tracking-wider w-32">Insolvency No</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-neutral-700 uppercase tracking-wider w-48">Name</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-neutral-700 uppercase tracking-wider w-32">IC No</th>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-neutral-700 uppercase tracking-wider w-24">RO Date</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-neutral-700 uppercase tracking-wider w-24">AO Date</th>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-neutral-700 uppercase tracking-wider w-24">Actions</th>
                                </tr>
                            </thead>
                            ...
                            @foreach($bankruptcies as $bankruptcy)
                                    <tr class="hover:bg-neutral-50 transition-colors duration-200">
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <span class="text-sm font-medium text-neutral-800">{{ $bankruptcy->insolvency_no }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <span class="text-sm text-neutral-900">{{ $bankruptcy->name }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <span class="text-sm text-neutral-800">{{ $bankruptcy->ic_no }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <span class="text-sm text-neutral-900">{{ $bankruptcy->others ?? 'N/A' }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <span class="text-sm text-neutral-900">{{ $bankruptcy->court_case_no ?? 'N/A' }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <span class="text-sm text-neutral-900">{{ $bankruptcy->formatted_updated_date }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <span class="text-sm text-neutral-900">{{ $bankruptcy->branch ?? 'N/A' }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('bankruptcy.show', $bankruptcy) }}" class="text-neutral-800 hover:text-neutral-700 transition-colors duration-200">View</a>
